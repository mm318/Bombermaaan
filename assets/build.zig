...
pub fn build(b: *std.Build) void {
    ...
    const lib = b.addStaticLibrary(.{
        .name = "bombermaaan_assets",
        .root_source_file = b.path("BombermaaanAssets.zig"),
        .target = target,
        .optimize = optimize,
    });

    const extract_assets_cmd = ExtractAssetsStep.create(b, b.path("game_assets.zip"), b.path("."));
    lib.step.dependOn(&extract_assets_cmd.step);

    lib.installHeader(b.path("BombermaaanAssets.h"), "BombermaaanAssets.h");
