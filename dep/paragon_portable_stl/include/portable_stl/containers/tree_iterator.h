...
// *
// * Project="Paragon Portable STL" File="tree_iterator.h"
// * 
...
// ***************************************************************************
#ifndef PSTL_TREE_ITERATOR_H
#define PSTL_TREE_ITERATOR_H
...
/**
 * @brief Iterator for nodes of a RB-tree.
 *
 * @tparam t_type Node's value type.
 * @tparam t_node_ptr
 * @tparam t_diff_type
 */
template<class t_type, class t_node_ptr, class t_diff_type> class tree_iterator {
  /**
   * @brief Node types.
   *
   */
...
public:
   ...
   *
   */
  using pointer           = typename t_node_types::t_node_value_type_pointer;

  /**
   * @brief Default constructor.
   *
   */
  tree_iterator() noexcept : m_ptr(nullptr) {
  }
   ...
   *
   * @return tree_iterator A copy of *this that was made before the change.
   */
  tree_iterator operator++(int) {
    tree_iterator iter(*this);
    ++(*this);
    return iter;
  }
   ...
   */
  friend bool operator==(tree_iterator const &lhv, tree_iterator const &rhv) {
...
private:
   ...
   * @param ptr
   */
  explicit tree_iterator(t_node_pointer ptr) noexcept : m_ptr(ptr) {
  }
   ...
   * @param ptr
   */
  explicit tree_iterator(t_end_node_pointer ptr) noexcept : m_ptr(ptr) {
  }
