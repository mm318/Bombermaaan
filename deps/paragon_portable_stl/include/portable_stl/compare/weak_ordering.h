...
// *
// * Project="Paragon Portable STL" File="weak_ordering.h"
// * 
...
// ***************************************************************************
#ifndef PSTL_WEAK_ORDERING_H
#define PSTL_WEAK_ORDERING_H
...
class weak_ordering final {
   ...
   */
  constexpr inline weak_ordering(::portable_stl::compare_helper::ordered const value) noexcept
      : m_value(static_cast<decltype(m_value)>(value)) {
  }

public:
  /**
   * @brief A valid value of the type weak_ordering indicating less-than (ordered before) relationship.
   * @return compare result.
   */
  constexpr inline static weak_ordering less() noexcept {
    return {::portable_stl::compare_helper::ordered::less};
  }

  /**
   * @brief A valid value of the type weak_ordering indicating equivalence (neither ordered before nor ordered after).
   *
   * @return compare result.
   */
  constexpr inline static weak_ordering equivalent() noexcept {
    return {::portable_stl::compare_helper::ordered::equivalent};
  }

  /**
   * @brief A valid value of the type weak_ordering indicating greater-than (ordered after) relationship.
   *
   * @return compare result.
   */
  constexpr inline static weak_ordering greater() noexcept {
    return {::portable_stl::compare_helper::ordered::greater};
  }

  /**
   * @brief weak_ordering is implicitly-convertible to partial_ordering.
   */
   ...
   */
  constexpr inline bool operator==(::portable_stl::weak_ordering const other) const noexcept {
...
/**
 * @brief Comparison operators are defined between values of literal 0 and weak_ordering type.
 *
...
constexpr inline static bool operator==(::portable_stl::compare_helper::unspecified const w,
                                        weak_ordering const                               v) noexcept {
  return (v.operator==(w));
}
/**
 * @brief Comparison operators are defined between values of literal 0 and weak_ordering type.
