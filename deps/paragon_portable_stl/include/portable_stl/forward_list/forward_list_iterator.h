...
// *
// * Project="Paragon Portable STL" File="forward_list_iterator.h"
// * 
...
// ***************************************************************************
#ifndef PSTL_FORWARD_LIST_ITERATOR_H
#define PSTL_FORWARD_LIST_ITERATOR_H
...
/**
 * @brief Iterator for nodes of a forward list.
 *
 * @tparam t_node_ptr
 */
template<class t_node_ptr> class forward_list_iterator {
  /**
   * @brief
   *
   */
   ...
   *
   */
  explicit forward_list_iterator(nullptr_t) noexcept : m_ptr(nullptr) {
  }
   ...
   */
  explicit forward_list_iterator(t_begin_node_pointer ptr) noexcept : m_ptr(t_traits::as_iter_node(ptr)) {
  }
   ...
   * @param ptr Given pointer.
   */
  explicit forward_list_iterator(t_node_pointer ptr) noexcept : m_ptr(t_traits::as_iter_node(ptr)) {
  }

  template<class, class> friend class forward_list;
  template<class> friend class forward_list_const_iterator;
...
public:
   ...
   *
   */
  using pointer           = ::portable_stl::rebind_pointer_t<t_node_pointer, value_type>;

  /**
   * @brief Default constructor.
   *
   */
  forward_list_iterator() noexcept : m_ptr(nullptr) {
  }
  ...
  /**
   * @brief Restur pointer to current element.
   *
   ...
   */
  friend bool operator==(forward_list_iterator const &lhv, forward_list_iterator const &rhv) {
...
#endif // PSTL_FORWARD_LIST_ITERATOR_H
